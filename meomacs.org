...
| variable | ETBookOT                | texts in markups                      |
...
#+begin_src emacs-lisp
  ...
  (defun meomacs-load-face-font (&optional frame)
    ...
    (let ((modeline-font-spec (meomacs--get-font-spec "modeline"))
      ...
      (set-face-attribute 'variable-pitch frame :font variable-font-spec :height 1.2)
      (set-face-attribute 'fixed-pitch frame :font fixed-font-spec)
      (set-face-attribute 'fixed-pitch-serif frame :font fixed-font-spec)
